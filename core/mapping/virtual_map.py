...
class VirtualMap:
    ...
    def __init__(self):
        ...
        self.ignored_portfolios: List[str] = []  # Track ignored portfolios
        self.is_frozen = False
        self.frozen_copy = None
        self.frozen_ignored = None

    def add_portfolio(
        self, name: str, base_bid: float, target_cpa: Optional[float] = None
    ):
    ...
    def add_ignored(self, name: str):
        """Add portfolio to ignored list"""
        if self.is_frozen:
            return

        if name not in self.ignored_portfolios:
            self.ignored_portfolios.append(name)

        # Remove from data if exists
    ...
    def merge_completion_template(
        self, df: pd.DataFrame, bulk_portfolios: List[str]
    ) -> Dict[str, str]:
        """
        ...
        for _, row in df.iterrows():
            ...
            if portfolio_name not in bulk_portfolios:
                errors[portfolio_name] = (
                    f"Portfolio '{portfolio_name}' does not exist in Bulk file"
                )
                continue

            # Handle Ignore
            if str(base_bid).strip().lower() == "ignore":
                self.add_ignored(portfolio_name)
            else:
                # Validate base_bid
                try:
                    ...
                    if base_bid_float < 0:
                        errors[portfolio_name] = (
                            f"Invalid Base Bid value for portfolio: {portfolio_name}"
                        )
                        continue
                except (ValueError, TypeError):
                    errors[portfolio_name] = (
                        f"Base Bid is required for portfolio: {portfolio_name}"
                    )
                    continue
                ...
                if target_cpa and str(target_cpa).strip():
                    try:
                        ...
                        if target_cpa_float < 0:
                            errors[portfolio_name] = (
                                f"Invalid Target CPA value for portfolio: {portfolio_name}"
                            )
                            continue
                    except (ValueError, TypeError):
                        errors[portfolio_name] = (
    ...
    def get_missing_portfolios(self, bulk_portfolios: List[str]) -> List[str]:
        """Get list of portfolios in bulk but not in virtual map (excluding ignored)"""
        vm_portfolios = set(self.data.keys())
        bulk_set = set(bulk_portfolios)
        ignored_set = set(self.ignored_portfolios)

        # Missing = in bulk, not in VM, not ignored
        missing = bulk_set - vm_portfolios - ignored_set
        return list(missing)
    ...
    def freeze(self):
        ...
        self.frozen_ignored = deepcopy(self.ignored_portfolios)
    ...
    def get_ignored(self) -> List[str]:
        """Get the ignored portfolios list"""
        if self.is_frozen and self.frozen_ignored:
            return self.frozen_ignored
        return self.ignored_portfolios
